...
{
  std::cout <<" Line " << __LINE__ << " Shadow created" << std::endl;
}
...
(std::string arg_name, std::string arg_fhandle, std::string arg_chunk_index)
{
  ...
  Json::Value myv;
  //std::cout <<" FFF " << arg_name << std::endl;

  try {
    std::cout <<" Line " << __LINE__ << " Inside the Shadow Directory.ObtainChunkURL" << arg_name << std::endl;

    myv = myClient.ObtainChunkURL("ObtainChunkURL",
        "This is a Directory JSON string!",
			  (this->class_id).c_str(),
         arg_fhandle,
         arg_name,
         arg_chunk_index,
    ...
    cout << "URLs" << myv.toStyledString() << endl;
  }
